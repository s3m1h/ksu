...
int karsilastirmaQ=0; // global deðisken -- quick sort algoritmasý karsilastirma sayisi icin
...
// uygulamamýzýn menüsü
void printMenu();

// bir diziyi baþka bir diziye kopyalamak için bir foksiyon
// ilk diziyi ikinci diziyi ve dizi boyutunu parametre olarak alýr.
void *diziKopyala(int ilkDizi[],int ikinciDizi[], int boyut);

// herhangi bir diziyi ekrana yazdýrmak için bir fonksiyon
// dizi ve dizi boyutunu parametre olarak alýr.
void diziPrint(int dizi[], int boyut);

// random sayi üreten bir fonksiyon
// mininum ve maksimum sayi araliði alýr.
int randNum(int min, int max);

// random sayilardan oluþan dizi üretme
// dizi, dizi boyutu, min max -- random sayi araliði
...
// dizi boyutu , random sayi için minimum deðer, random sayi için maximum deðer
void algoritmaIslem(int boyut, int min, int max);

// metinleri renklendirmek için kullanýlacak baþlangýç kodlarý 
//-------------------------------------------------
HANDLE hConsole(){
...
int main(){
	...
	while(true){
		...
		if(secim == 'q'){
			...
			scanf("%d",&diziBoyutu);
			SetConsoleTextAttribute(hConsole(),BACKGROUND_GREEN);
			printf("Uretilecek dizi icin rastgele sayilarin araliðini sirayla belirtin..\n>");
			SetConsoleTextAttribute(hConsole(), saved_attributes);
			printf("(min) \n>");
			scanf("%d",&min);
			printf("(max) \n>");
...
void algoritmaIslem(int boyut,int min, int max){
	...
	// satirin arka plan rengini deðiþtirme 
	SetConsoleTextAttribute(hConsole(),BACKGROUND_GREEN);
